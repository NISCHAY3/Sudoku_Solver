    ...
    for (let i = 0; i < 81; i++) {
        ...
        funcId("board").children[i].addEventListener("click", async function () {
            ...
            if (funcId("board").children[i].innerHTML != '' && numToAdd == 'del') {
                ...
                funcId("board").children[i].classList.remove("solveColor");       // This entire 'if' is created to delete an element from the board. The deleteChecker is set to 1 after this so that in the next if, "delete" will not be placed in the board.
            ...
            if (checkDuplicates(board, parseInt(numToAdd), finalIndexes) == true && numToAdd != undefined && numToAdd != 'del') {
                ...
                funcId("board").children[i].classList.add("solveColor")        // Adds number to board after knowing that it follows the rule of sudoku and does not get repeated in the same row, column or sub-square
